...
\begin{lstlisting}[caption=Wczytywanie danych MNIST, language=R, label={lst:r_data}]
mnist <- dataset_mnist()
...
Przygotowane dane wejœciowe s¹ w postaci 3-wymiarowej tablicy, wiêc nale¿y zredukowaæ liczbê wymiarów oraz dodatkowo przeskalowaæ, aby znalaz³y siê one w przedziale <0;1>.

\begin{lstlisting}[caption=Przygotowanie danych wejœciowych, language=R, label={lst:r_data}]
...
\begin{lstlisting}[caption=Przygotowanie danych wyjœciowych, language=R, label={lst:r_data_out}]
...
\begin{lstlisting}[caption=Utworzenie modelu, language=R, label={lst:r_model}]
