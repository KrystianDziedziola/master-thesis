...
Biblioteka Keras najczêœciej u¿ywana jest w po³¹czeniu z jêzykiem Python, jednak istnieje równie¿ mo¿liwoœæ wykorzystania jej w jêzyku R.
...
Po uruchomieniu œrodowiska RStudio nale¿y zainstalowaæ bibliotekê keras (patrz listing \ref{lst:r_install_keras}).
...
W tym przyk³adzie zostanie u¿yty klasyczny zbiór danych MNIST, który zawiera obrazki w skali szaroœci o rozmiarach 28x28 pixeli przedstawiaj¹ce odrêcznie pisane cyfry wraz z odpowiadaj¹cymi im etykietami (patrz rysunek \ref{fig:mnist}).
...
W celu wczytania wartoœci wystarczy u¿yæ funkcji \textit{dataset\_mnist()} i przypisaæ wartoœci do zmiennej, a nastêpnie wydzieliæ odpowiednie zbiory treningowe oraz testowe (patrz listing \ref{lst:r_data}).
...
Przygotowane dane wejœciowe s¹ w postaci 3-wymiarowej tablicy, wiêc nale¿y zredukowaæ liczbê wymiarów oraz dodatkowo przeskalowaæ, aby znalaz³y siê one w przedziale <0;1> (patrz listing \ref{lst:r_reshape}).
...
Dane wyjœciowe nale¿y zakodowaæ za pomoc¹ kodu "1 z n" \textit{(ang. one-hot encoding)} (patrz listing \ref{lst:r_data_out}).
...
Na ostatniej warstwie u¿yta zosta³a funkcja aktywacji \textit{softmax}, która zwróci pradopowobieñstwo zajœcia jednego z 10 stanów  (patrz listing \ref{lst:r_model}).
...
Po utworzeniu modelu wywo³ywana jest metoda \textit{fit()}, która rozpoczyna proces uczenia (patrz listing \ref{lst:r_training}). 
Zastosowany zosta³ podzia³ na zbiory ucz¹cy i validacyjny w stosunku 4:1.

\begin{lstlisting}[caption=Rozpoczêcie procesu uczenia, language=R, label={lst:r_training}, float=!h]
history <- model %>% fit(
  x_train, y_train, 
...
Sprawdzenie skutecznoœci modelu na danych testowych odbywa siê za pomoc¹ funkcji \textit{evaluate()} pokazanej na listingu \ref{lst:r_evaluate}.
...
Z pewnoœci¹ bilioteka Keras mo¿e byæ z powodzeniem u¿ywana równie¿ w jêzyku R, gdy¿ oferuje ona takie same mo¿liwoœci, jak dla jêzyka Python.
Nale¿y jednak pamiêtaæ, ¿e u¿ywanie Keras'a w po³¹czeniu z jêzykiem Python jest o wiele bardziej popularne. 
Z tego powodu istnieje o wiele wiêcej poradników oraz artyku³ów opisuj¹cych wykorzystanie w³aœnie tej kombinacji narzêdzi, co mo¿e byæ pomocne przy implementacji bardziej z³o¿onych rozwi¹zañ.