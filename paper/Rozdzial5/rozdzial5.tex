...
Jako optymalizator u¿yty zosta³ \textit{SGD} (\textit{ang. stochastic gradient descent}) z funkcj¹ strat binarnej entropii krzy¿owej \textit{(ang. binary crossentropy)}.
...
Wyniki by³y obiecuj¹ce, wiêc sieæ tego typu zosta³a wziêta pod uwagê w dalszych pracach polegaj¹cych na optymalizacji sieci w celu uzyskania jak najlepszych wyników \ref{optimization}.
...
Analogicznie do modelu sieci z filtrem jednowymiarowym zosta³ stworzony podobny z 2-wymiarowym filtrem \ref{lst:conv-2d}

\begin{lstlisting}[caption=Model splotowej sieci neuronowej z 2-wymiarowym filtrem, language=Python, label={lst:conv-2d}]
model = Sequential()
...
W nastêpnym kroku przetestowana zosta³a rekurencyjna sieæ neuronowa. Pocz¹tkowy model sieci, który zosta³ utworzony zaprezentowany zosta³ na listingu \ref{lst:recursive}. 

\begin{lstlisting}[caption=Model rekurencyjnej sieci neuronowej, language=Python, label={lst:recursive}]
model = Sequential()
...
Jest to wynik o wiele gorszy w porównaniu do sieci konwolucyjnych testowanych w poprzednich rozdzia³ach. Próby rozbudowy i prostej optymalizacji sieci nie przynosi³y skutków, dlatego zosta³a ona pominiêta w dalszych badaniach.
...
\label{optimization}
Sieæ typu konwolucyjnego z filtrem 2-wymiarowym osi¹ga³a najlepsze wyniki testuj¹c prototypy ró¿nych typów sieci neuronowych. To w³aœnie tej architekturze zosta³o wiêc poœwiêcone najwiêcej pracy maj¹cej na celu optymalizacjê i polepszenie otrzymywanych wyników. 
...
\begin{lstlisting}[caption=Regu³a modyfikacji wspó³czynnika uczenia, language=Python, label={lst:learning_rate_scheduler}]
def step_decay(epoch):    
    initial_lrate=0.1
    drop=0.6
    epochs_drop = 10.0
...
Dynamiczna zmiana wspó³czynnika uczenia pozwoli³a na czêœciowe zredukowanie nadmiernego dopasowania sieci, jednak sieæ wci¹¿ zaobserwowaæ mo¿na jej przeuczanie. Mimo to skutecznoœæ zosta³a poprawiona do blisko \textbf{80,5\%} \ref{fig:adaptive_learning_rate}.
\begin{figure}[h!]
	\centering
...
\begin{lstlisting}[caption=Ostateczny model sieci konwolucyjnej, language=Python, label={lst:conv_final}]
model = Sequential()
...
Na modelu wywo³ana zosta³a metoda \textit{evaluate}, której parametrami by³y dane wejœciowe oraz wyjœciowe ze zbioru testowego. Wynikiem wykonania tych poleceñ \ref{lst:test_verification} jest skutecznoœæ modelu na danych, które zosta³y u¿yte po raz pierwszy \ref{lst:test_results}.
...
Osi¹gniête wyniki nie s¹ identyczne jak te uzyskane na danych walidacyjnych, jednak s¹ one wystarczaj¹co podobne, aby stwierdziæ, ¿e model dobrze radzi sobie z nowymi danymi. Wynik na poziomie prawie \textbf{77\%} (76.8927\%) na danych testowych jest zadowalaj¹cy i przekracza za³o¿ony próg 70\% \ref{expectation}.
...
\item Przetworzenie danych do innej postaci (\textit{ang. feature extraction}) np. wizualizacja aktywnoœci mózgu \cite{brain_visualization} lub stworzenie tzw. \textit{heat map}'y metod¹ \textit{matching pursuit} \cite{matching_pursuit}
\item Wykorzystanie gotowych modeli sieci wbudowanych w bibliotekê \textit{Keras} np. \textit{Xception}, \textit{ResNet}, \textit{VGG19} itd. \cite{keras_applications} 
\end{itemize}
